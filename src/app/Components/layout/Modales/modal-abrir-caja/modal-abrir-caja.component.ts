 ...
 usuarioSeleccionado!: Usuario | null;
  ...
  guardarEditar_Caja() {
    ...
    const cajaAbiertaUsuario = this.listaCaja.find(caja => {
      console.log(`Verificando caja con idUsuario ${caja.idUsuario} y estado ${caja.estado}`);
      return caja.idUsuario === idUsuario && caja.estado === 'Abierto';
    });
    if (precio <= 0) {
      // Si el usuario ya tiene una caja abierta, mostrar un mensaje de error
      Swal.fire({
        icon: 'error',
        title: 'Error',
    ...
    console.log('Caja Abierta para el Usuario:', cajaAbiertaUsuario);
    if (cajaAbiertaUsuario) {
      // Si el usuario ya tiene una caja abierta, mostrar un mensaje de error
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Ya tienes una caja abierta con este usuarios. Debes cerrarla antes de abrir otra.',
      });
      return;
    }
    ...
    const _Caja: Caja = {
      ...
      nombreUsuario: this.formularioCaja.value.nombreCompleto,
      comentariosGastos: "",
      comentariosDevoluciones: "",
      transaccionesTexto: "0",
    };
